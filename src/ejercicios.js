...
function rotar90Grados(matriz) {
  ...
  const resultado = Array.from({ length: columnas }, () => Array(filas));
...
function mezclarImagenes(matriz1, matriz2, factor) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      resultado[i][j].r = limitarValorColor(matriz1[i][j].r * (1 - factor) + matriz2[i][j].r * factor);
      resultado[i][j].g = limitarValorColor(matriz1[i][j].g * (1 - factor) + matriz2[i][j].g * factor);
      resultado[i][j].b = limitarValorColor(matriz1[i][j].b * (1 - factor) + matriz2[i][j].b * factor);
    }
  }
 ...
 function copiarMatriz(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      ...
      resultado[i][j].r = limitarValorColor(0.393 * r + 0.769 * g + 0.189 * b);
      resultado[i][j].g = limitarValorColor(0.349 * r + 0.686 * g + 0.168 * b);
      resultado[i][j].b = limitarValorColor(0.272 * r + 0.534 * g + 0.131 * b);
    }
  }
